...
/// <summary>
/// 对话触发器管理器：可视化总览场景中的局部触发器（DialogueTrigger / DialogueDemoTrigger）
/// + 快速接线（换对话/改触发方式，SerializedProperty 路线免费 Undo）+ 定位 Ping / ▶ 预览 / ✕ 移除。
/// hierarchy 变化自动刷新。视觉小说的全局串播入口（剧本导演）在 vn 扩展包的 Director Manager。
/// </summary>
public class DialogueTriggerWindow : EditorWindow
{
    ...
    {
        var window = GetWindow<DialogueTriggerWindow>("对话触发器管理器");
        window.Refresh();
    }
    ...
    private void OnGUI()
    {
        ...
        if (_entries.Count == 0)
        {
            EditorGUILayout.HelpBox(
                "当前场景没有任何对话触发器。\n选中物体点上方「＋ 添加触发器」，或手动 Add Component > Dialogue > Dialogue Trigger。\n"
                + "（视觉小说的全局章节串播请安装 com.otus.dialogue.vn 扩展包，用 Dialogue > Director Manager。）", MessageType.Info);
            return;
        }
    ...
    private static void AddToSelection()
    {
        ...
        if (added > 0)
        {
            Debug.Log($"[Dialogue] 已给 {added} 个物体挂 DialogueTrigger（默认「靠近 + 按键」，已补 Trigger BoxCollider）。"
                + "在「对话触发器管理器」或 Inspector 里接线对话资产，改「自动播放」模式可去掉交互。记得保存场景。");
        }
    }
